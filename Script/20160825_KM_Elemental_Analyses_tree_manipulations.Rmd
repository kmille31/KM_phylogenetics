---
title: "KAM Elemental Analyses and tree manipulations (PCOA)"
author: "Katie A. Miller and Antdrew D. Nguyen"
date: "2016-August-25"
output: pdf_document
---
```{r libraries}
...
#Libraries needed for extracting BioClim data
library(raster)
library(rgdal)
library(dismo)
library(maptools)
...
#Combining datasets: CN with P; adding climate data
...
#Ploting first bioclim dimension (MAT) to check download worked.
plot(BioClim,1,col=colorRampPalette(c("blue","purple","red"))(255))

# Extract climate variable (MAT) for site lon/lat from bioclim data
#Note that Bioclim data have MAT multiplied by 10 for ease of analysis
BioClim_All<- extract(BioClim, combined[,c("Lon","Lat")])


dbio <- cbind(combined, BioClim_All)
head(dbio)
tail(dbio)

#Combining Common Garden data (CN and P)
CG_CN<-read.csv("../Data/20160826_CommonGarden_CN.csv")
CG_P<-read.csv("../Data/20160826_CommonGarden_Phos.csv")

CG_Combined<-left_join(CG_CN,CG_P,by=c("Site","ColonyID","rep","TimePoint","SampleID","Colony"))
write.csv(CG_Combined,"../Data/20160826_CG_combined_CN_P.csv")
head(CG_Combined)
...
##This is important: creating a new column with the colony names (the tip labels in my tree).
pcoa.dat$Name<-KMtree2$tip.label
